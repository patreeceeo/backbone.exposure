describe 'Backbone.Accessors', ->

  it 'should be defined', ->
    expect(Backbone.Accessors).toBeDefined()
  ...
  describe 'the methods created for attributes', ->
    ...
    beforeEach ->
      class Actor extends Backbone.Accessors.Model
        defaults:
          color: 'blue'
          name: 'tobias'
          age: 34
  ...
  describe 'the attributes created for methods', ->
    ...
    it "should try to invoke a synonymous method for each key in the initial attributes, arguing the associated value", ->
      method = sinon.spy()
      ...
      expect(method).toHaveBeenCalledWith('now')
